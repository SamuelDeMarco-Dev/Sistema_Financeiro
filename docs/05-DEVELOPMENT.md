...
A versão do Node é fixada em `.nvmrc` (`22`) e em `engines` do `package.json`. Divergência de _major_ causa diferenças reais de comportamento em `Intl`, _streams_ e resolução de módulos — o CI valida.
...
Apenas variáveis com prefixo `VITE_` chegam ao _bundle_ — e tudo que chega ao _bundle_ é **público**. Nenhum segredo, chave de API ou credencial no frontend, em nenhuma hipótese.
...
| Script             | Comando                                                                    | Uso                                |
| ------------------ | -------------------------------------------------------------------------- | ---------------------------------- |
| `dev`              | `tsx watch src/index.ts`                                                   | Desenvolvimento com _hot reload_   |
| `build`            | `tsc -p tsconfig.build.json`                                               | Compila para `dist/`               |
| `start`            | `node dist/index.js`                                                       | Execução do artefato               |
| `lint`             | `eslint . --max-warnings 0`                                                | Lint (zero _warnings_)             |
...
| `teste:observar`   | `vitest`                                                                   | Modo _watch_                       |
...
`npm run verificar` reproduz exatamente o que o CI executa. Rodá-lo antes do _push_ é a diferença entre descobrir um problema em 30 segundos ou em 6 minutos.
...
| Hooks                         | **inglês** com `use` | `camelCase`                           | `useMovimentacoes()`                         |
...
Hooks são a única exceção ao domínio em pt-BR (fora as próprias palavras-chave de linguagem/bibliotecas): `eslint-plugin-react-hooks` reconhece um Hook customizado pelo prefixo `use` via expressão regular fixa (`/^use[A-Z0-9]/`), sem opção de configuração — um Hook chamado `usarAlgo` é lido como função comum, e chamadas a outros Hooks dentro dele disparam falso-positivo de `react-hooks/rules-of-hooks`. Restante do domínio (parâmetros, retorno, lógica) continua em pt-BR normalmente: `useMovimentacoes(filtros: FiltroMovimentacao)`.
...
Motivo: caracteres não-ASCII em identificadores criam problemas reais em _tooling_, _locale_ de banco, _encoding_ de terminal e nomes de arquivo entre Windows e Linux. Acentuação é obrigatória apenas em **texto exibido ao usuário** (mensagens, rótulos, documentação).
...
**Sem _default export_** (exceto onde a ferramenta exige: rotas Express, arquivos de configuração). _Named exports_ permitem renomeação segura e busca confiável.
...
export const criarMovimentacaoSchema = z.object({
  body: z
    .object({
      ...
      valor: z
        .string()
        .regex(/^\d+(\.\d{1,2})?$/, 'Valor inválido. Use o formato 1234.56.')
    ...
    <article className="flex items-center gap-3 rounded-md border border-borda bg-superficie p-4">
      <div className="min-w-0 flex-1">
        ...
        <p className="text-sm text-textoSuave">
          {formatarData(movimentacao.dataCompetencia)} ·{' '}
  ...
  return (
    <ul>
      {data.movimentacoes.map((m) => (
        <CartaoMovimentacao key={m.id} movimentacao={m} />
      ))}
    </ul>
  );
}
...
`endOfLine: "lf"` mais `.gitattributes` com `* text=auto eol=lf` evita o clássico _diff_ inteiro de fim de linha entre Windows e Linux.
...
`--max-warnings 0`: _warning_ é erro. _Warning_ tolerado acumula até ninguém mais ler a saída do lint.
...
| --------- | ---------------------------------------------------------------- | ------------------------------------------------------- |
| `main`    | **Produção.** Todo merge dispara deploy automático               | PR obrigatório, CI verde, 1 aprovação, sem _force push_ |
| `staging` | **Desenvolvimento/homologação.** Integração contínua do trabalho | CI verde, sem _force push_                              |
...
_Rebase_ em cima de `staging` antes do PR (em vez de merge) mantém o histórico linear e o _diff_ do PR contendo só o que a issue mudou.
...
O merge em `main` dispara o pipeline de produção (build → testes → imagem → deploy SSH → _health check_ → _rollback_ se falhar). Ver [08-CICD.md](08-CICD.md).
...
3. Corpo explica **por que**, não o que — o _diff_ já mostra o que.
...
1. **Correção da regra de negócio.** A regra está implementada como a especificação define? Casos-limite (zero, negativo, arredondamento, virada de mês, _timezone_)?
...
8. **Legibilidade.** Nomes conforme §4? Comentários explicam _por que_?
...
| Escopo                        | Mínimo   |
| ----------------------------- | -------- |
| Global (_statements_)         | 80%      |
| Global (_branches_)           | 75%      |
| `src/servicos/**`             | **90%**  |
...
describe('MovimentacaoServico.criar', () => {
  ...
  it('lança ProibidoErro quando a conta é de outro usuário (RN-51)', async () => {
    ...
    await expect(servico.criar('u1', fabricarDadosDespesa())).rejects.toThrow(ProibidoErro);
  });
...
**Convenções:** nome de teste descreve **comportamento**, não implementação; um `expect` conceitual por teste; regra de negócio testada cita o ID; dados vêm de _factories_ (`testes/fabricas/`), nunca literais duplicados.
...
| 2   | Regra de negócio em controlador                  | Impossível testar sem HTTP; duplica quando surge segundo _caller_ |
...
| 16  | Identificador com acento ou cedilha              | Problemas de _tooling_ e _encoding_ (§4.2)                        |
