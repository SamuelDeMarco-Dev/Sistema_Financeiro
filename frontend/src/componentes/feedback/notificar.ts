...
function emitir(): void {
  ouvintes.forEach((ouvinte) => ouvinte(notificacoes));
}
...
function adicionar(tipo: TipoNotificacao, mensagem: string): void {
  ...
  emitir();
  setTimeout(() => removerNotificacao(notificacao.id), DURACAO_MS);
}
...
export const notificar = {
  sucesso: (mensagem: string): void => adicionar('sucesso', mensagem),
  erro: (mensagem: string): void => adicionar('erro', mensagem),
};
