...
describe('useAtualizarFoto', () => {
  it('mescla a nova fotoUrl no cache existente e sincroniza o usuario da sessao', async () => {
    clienteConsulta.setQueryData(CHAVES_CONSULTA.perfil, PERFIL_FAKE);
    ...
    await waitFor(() => {
      expect(clienteConsulta.getQueryData<PerfilCompleto>(CHAVES_CONSULTA.perfil)?.fotoUrl).toBe(
        'https://exemplo.com/nova.webp',
      );
    });
    // O resto do perfil em cache continua intacto — so fotoUrl muda.
    expect(clienteConsulta.getQueryData<PerfilCompleto>(CHAVES_CONSULTA.perfil)?.nome).toBe(
      'Samuel De Marco',
    );
  ...
  it('nao quebra quando o cache de perfil ainda nao foi carregado', async () => {
    ...
    expect(clienteConsulta.getQueryData(CHAVES_CONSULTA.perfil)).toBeUndefined();
  });
});
