...
const Inicio = lazy(() => import('@/paginas/Inicio').then((modulo) => ({ default: modulo.Inicio })));
const NaoEncontrada = lazy(() =>
  import('@/paginas/NaoEncontrada').then((modulo) => ({ default: modulo.NaoEncontrada })),
);

function EsqueletoPagina(): JSX.Element {
  return (
...
function ComSuspense({ children }: { children: ReactNode }): JSX.Element {
